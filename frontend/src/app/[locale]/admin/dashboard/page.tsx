...
export default function AdminDashboard() {
  ...
  const revenueStats = [
    ...
    { label: t("allTime"), value: stats?.revenue?.allTime || 0, color: "from-zinc-800 to-black" },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      <DashboardHeader
        ...
        roleBadge={{ label: t("admin"), type: "admin" }}
      />
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-bold text-zinc-900">{t("revenueOverview")}</h2>
        </div>
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-bold text-zinc-900">{t("platformStats")}</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-bold text-zinc-900">{t("alerts")}</h2>
        </div>
        ...
        {!hasAlerts ? (
          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-8 text-center">
            ...
            <h3 className="text-lg font-bold text-emerald-900">{t("allCaughtUp")}</h3>
              ...
              <div className="bg-white border border-zinc-100 rounded-2xl overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-zinc-900">{t("pendingContracts")}</span>
                  </div>
                  <span className="bg-amber-100 text-amber-700 text-[10px] font-black px-2 py-0.5 rounded-full uppercase">
                    {stats.alerts.pendingContracts.length} {t("waiting")}
                  </span>
                </div>
            ...
            {stats?.alerts?.pendingOrdersCount > 0 && (
              <div className="bg-white border border-amber-100 rounded-2xl p-6 flex flex-col justify-between shadow-sm shadow-amber-100/20">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-zinc-900">{t("totalOrders")}</h3>
                    <p className="text-sm text-zinc-500 mt-1">
                      {t("pendingOrdersCount", { count: stats.alerts.pendingOrdersCount })}
                    </p>
                  </div>
                </div>
                <Link
