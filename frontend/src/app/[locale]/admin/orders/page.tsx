        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-end gap-3 w-full xl:w-auto">
            ...
            <div className="flex flex-col gap-1.5 flex-1 sm:flex-none">
              <span className="text-[10px] font-black text-zinc-400 uppercase tracking-tighter ml-1">{t("from")}</span>
            ...
            {/* Date To */}
            <div className="flex flex-col gap-1.5 flex-1 sm:flex-none">
              <span className="text-[10px] font-black text-zinc-400 uppercase tracking-tighter ml-1">{t("to")}</span>
              <div className="flex items-center gap-2 px-3 py-2 bg-zinc-50 border border-zinc-100 rounded-xl hover:bg-zinc-100/50 transition-colors h-[42px]">
                <input 
                  type="date" 
                  value={dateTo}
           ...
           {/* Status Filter */}
            ...
            >
              ...
              <span className="whitespace-nowrap">{exporting ? t("exporting") : t("export")}</span>
            </button>
      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-end pr-2">
          <div className="flex flex-col items-end leading-tight">
            <span className="text-[12px] font-black text-zinc-600 uppercase tracking-widest -mt-1"> {t("ordersCount", { count: totalCount })}</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-zinc-100 rounded-3xl p-12 flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-sm font-bold text-zinc-400 uppercase tracking-widest">{t("loadingOrders")}</p>
        </div>
      ) : error ? (
        ...
        <div className="bg-white border border-zinc-200 rounded-3xl overflow-hidden shadow-sm">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50 border-b border-zinc-100">
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-500">{t("order")}</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-500">{t("dealer")}</th>
                  ...
                  <tr key={order.id} className="hover:bg-zinc-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                       <div className="flex items-center gap-1.5 text-xs font-bold text-zinc-700">
                          ...
                          {format(new Date(order.createdAt), "MMM d, yyyy", { locale: locale === 'tr' ? tr : undefined })}
                        </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter border ${getStatusStyle(order.status)}`}>
                        {t(order.status === "pending_payment" ? "pendingPayment" : order.status)}
                      </div>
                    </td>
                    <td className="px-6 py-5">
          ...
          {filteredOrders.length === 0 && (
            <div className="p-12 text-center">
               <p className="text-sm font-bold text-zinc-400 uppercase tracking-widest">{t("noOrders")}</p>
            </div>
          )}
          <Pagination
            page={page}
