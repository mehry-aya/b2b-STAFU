...
export default function AdminDashboard() {
  ...
  const revenueStats = [
    ...
    { label: "All Time", value: stats?.revenue?.allTime || 0, color: "from-zinc-800 to-black" },
  ];
  ...
  const platformStats = [
    ...
    { label: "Total Admins", value: stats?.platform?.totalAdmins || 0, icon: Anchor, href: "/admin/export" },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      <DashboardHeader
        ...
        subtitle="Platform-wide management for STAFUPRO B2B. Oversee revenue, dealers, and order fulfillment."
        icon={Anchor}
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-bold text-zinc-900">Alerts & Action Items</h2>
        </div>
        ...
        {!hasAlerts ? (
          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-8 text-center">
            ...
            <p className="text-emerald-700 mt-1">No pending contracts or orders needing immediate attention.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white border border-zinc-100 rounded-2xl overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-zinc-900">Pending Contracts</span>
                  </div>
                  <span className="bg-amber-100 text-amber-700 text-[10px] font-black px-2 py-0.5 rounded-full uppercase">
                    {stats.alerts.pendingContracts.length} Waiting
                  </span>
                </div>
            ...
            {stats?.alerts?.pendingOrdersCount > 0 && (
              <div className="bg-white border border-amber-100 rounded-2xl p-6 flex flex-col justify-between shadow-sm shadow-amber-100/20">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-500 mt-1">
                      There are <span className="font-bold text-amber-600">{stats.alerts.pendingOrdersCount}</span> orders waiting for payment confirmation or approval.
                    </p>
                  </div>
                </div>
                <Link
                  href="/admin/orders"
                  className="mt-6 flex items-center justify-center gap-2 w-full bg-amber-600 text-white font-bold py-3 rounded-xl hover:bg-amber-700 transition-colors shadow-lg shadow-amber-200"
                >
                  View Orders
